...
export const getDashboardData = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [{ data: profiles }, { data: devices }, { data: schedule }, { data: invites }] =
      await Promise.all([
        context.supabase.from("profiles").select("id, status, full_name, job_role, departments(name)").eq("org_id", orgId),
        context.supabase.from("devices").select("id, name, os, agent_version, status, monitoring_state, last_heartbeat_at, last_sync_at, profiles(full_name, job_role, departments(name))").eq("org_id", orgId),
        context.supabase.from("monitoring_schedules").select("*").eq("org_id", orgId).is("profile_id", null).order("day_of_week"),
        context.supabase.from("invitations").select("id, status").eq("org_id", orgId).in("status", ["pending", "sent"]),
      ]);
    ...
    const byDate = new Map<string, { date: string; productivity: number; focus: number; productive: number; distracted: number; idle: number }>();
    for (const s of summaries as any[]) {
      const b = byDate.get(s.date) ?? { date: s.date, productivity: 0, focus: 0, productive: 0, distracted: 0, idle: 0 };
    ...
    return {
      ...
      kpis: {
        ...
        devices_online: (devices ?? []).filter((d: any) => d.monitoring_state === "active" || d.monitoring_state === "off_shift").length,
...
export const getAppUsage = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: sessions, error } = await context.supabase
      ...
      .select("app_name, process_name, category, is_idle, duration_seconds, profiles(full_name)")
      .eq("org_id", orgId)
...
export const getActivitySessions = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    let q = context.supabase
      .from("activity_sessions")
      .select("id, app_name, window_title, category, is_idle, started_at, duration_seconds, profiles(full_name), devices(name)")
      .eq("org_id", orgId)
      .gte("started_at", since)
...
export const getReportsData = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: reports } = await context.supabase
      ...
      .limit(30);
    return { org, summaries: summaries ?? [], aiReports: reports ?? [] };
  });
